...
int gSensorHeight = 525;
int gSensorWidth = 525;
int gMargin = 10;
...
float displayXLoc, displayYLoc,displayXSize;
...
void drawSquare() {
  ...
  for(int i = 0; i < gCurrentNumTouches; i++) {
      for(int j = 0; j < gCurrentNumHTouches; j++) {
         displayXLoc = map(gTouchHLocations[j], 0, gMaxTouchLocation, 0, gSensorWidth);
         displayYLoc = map(gTouchLocations[i], 0, gMaxTouchLocation, gSensorHeight, 0);
        
         displayXSize = map(gTouchHSizes[j], 0, gMaxTouchSize, 
        ...
        noStroke();
        fill(0, 0, 255);
        ellipse(displayXLoc + gMargin, displayYLoc + gMargin, displayXSize, displayXSize);
